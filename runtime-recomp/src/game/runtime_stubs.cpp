...
extern "C" void dkr_skybox_cover_begin(std::uint8_t* rdram,
    ...
    // matrices. Uniformly zoom projected X/Y for cover behaviour, while
    // retaining Z/W and therefore the original depth relationship.
    for (std::uint32_t row = 0; row < 4U; ++row) {
        for (std::uint32_t column = 0; column < 2U; ++column) {
            ...
            WriteRdramFloat(rdram, address,
                            g_saved_sky_projection_columns[slot] * cover_zoom);
        }
    }
    g_sky_cover_active = true;
    if (!g_logged_sky_cover.exchange(true, std::memory_order_relaxed)) {
        std::fprintf(stderr, "[boot][widescreen] skydome local cover zoom=%.3f\n",
                     cover_zoom);
    }
#else
    (void)rdram;
