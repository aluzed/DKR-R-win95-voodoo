...
dkr::sync::mutex g_input_backend_transition_mutex;
...
dkr::sync::mutex g_sdl3_probe_result_mutex;
...
std::filesystem::path RuntimeInputHostPath() {
    if (const char* override_path = std::getenv("DKR_SDL3_INPUT_HOST");
        ...
        std::error_code error;
        if (dkr::fs::is_regular_file(candidate, error)) return candidate;
    }
#if defined(_WIN32)
...
int LoadControllerMappings(const std::filesystem::path& path,
                           ...
                           const char* label) {
    std::error_code error;
    if (path.empty() || !dkr::fs::is_regular_file(path, error)) {
...
bool WriteUserMappings(const std::vector<std::string>& lines,
    ...
    std::error_code error;
    dkr::fs::create_directories(g_user_mapping_path.parent_path(), error);
    if (error) {
    ...
    dkr::fs::remove(g_user_mapping_path, error);
    error.clear();
    dkr::fs::rename(temporary, g_user_mapping_path, error);
    if (error) {
...
void CancelSdl3InputProbe() {
    ...
    {
        dkr::sync::scoped_lock lock(g_sdl3_probe_result_mutex);
...
void CompleteSdl3InputProbe() {
    ...
    dkr::sync::unique_lock transition_lock(g_input_backend_transition_mutex);
...
void EnsureInputBackendHealthy() {
    ...
    dkr::sync::scoped_lock transition_lock(g_input_backend_transition_mutex);
    if (g_active_input_backend.load(std::memory_order_acquire) !=
            dkr::runtime::platform::InputBackend::SDL3Native ||
        g_sdl3_input_client.healthy()) {
        return;
...
bool dkr::runtime::platform::export_controller_mappings(
    ...
    dkr::sync::scoped_lock lock(g_platform_mutex);
    std::error_code error;
    if (!dkr::fs::is_regular_file(g_user_mapping_path, error)) {
        status = "No custom controller mappings have been created yet.";
        return false;
    }
    dkr::fs::copy_file_overwrite(g_user_mapping_path, destination, error);
    if (error) {
