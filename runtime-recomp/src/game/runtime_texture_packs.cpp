...
dkr::sync::mutex g_refresh_mutex;
...
std::uintmax_t ManagedSize(const std::filesystem::path& path) {
    std::error_code error;
    if (dkr::fs::is_regular_file(path, error)) {
        const auto size = dkr::fs::file_size(path, error);
        return error ? 0U : size;
    }
    error.clear();
    if (!dkr::fs::is_directory(path, error)) return 0U;
...
void SavePackIndexLocked() {
    ...
    std::error_code error;
    dkr::fs::create_directories(g_index_path.parent_path(), error);
    if (error) return;
    ...
    dkr::fs::rename(temporary, g_index_path, error);
    if (!error) return;
    error.clear();
    dkr::fs::remove(g_index_path, error);
    error.clear();
    dkr::fs::rename(temporary, g_index_path, error);
...
bool EnsureManagedRiceCoordinatePolicy(const std::filesystem::path& database_path,
    ...
    configuration["defaultShift"] = std::string(
        dkr::runtime::rice_texture::kLegacyCoordinateShift);
    std::ofstream database_file(database_path.string(), std::ios::trunc);
    if (!database_file) {
...
PackInfo PlaceholderInfo(const std::filesystem::path& path, bool directory) {
    ...
    if (directory) {
        std::error_code error;
        info.format = dkr::fs::is_regular_file(
...
void ScanLibrary(bool cache_only, bool force_deep,
    ...
    dkr::sync::scoped_lock refresh_lock(g_refresh_mutex);
    ...
    const auto populate_metadata = [&](PackInfo& info, bool allow_deep_size) {
        ...
        if (managed_size != managed_sizes.end()) {
            info.managed_size_bytes = managed_size->second;
                   ...
                   dkr::fs::is_regular_file(info.path)) {
