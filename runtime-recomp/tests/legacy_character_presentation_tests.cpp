...
int main(){try {
 ...
 for(unsigned i=0;i<2;++i){AllocatedCharacter a;a.id=std::string(64,'a'+i);a.name="Custom";a.base_character=3;a.portrait=906+i;
  ...
  for(unsigned k=0;k<17;++k)a.race_audio.cues[k]={{k+1,90,110,63},12,14000};assets->characters.push_back(a);}
 ...
 for(unsigned i=0;i<2;++i)for(unsigned c=0;c<17;++c) {
 ...
 ctx=original;ctx.r4=0x165;check(!p.play(memory,ctx,calls,assets->characters,0));check(points==34 && plays==68);
